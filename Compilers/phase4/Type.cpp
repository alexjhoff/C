...
bool Type::isCompatable(const Type &right) const
{
    Type r = right.promote();
    Type t = this->promote();
    if(r.isScalar())
        r = Type(r._specifier, 0);
    if(t.isScalar())
        t = Type(t._specifier, 0);
    if (t == r)
        return true;
    return false;
}

bool Type::isCompareable(const Type &right) const
{
    Type r = right.promote();
    Type t = this->promote();
    if(r.isSimple() && t.isSimple()){
        if (t == r)
            return true;
    }
    return false;
